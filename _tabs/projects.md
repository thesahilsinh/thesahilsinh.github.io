---
layout: tab
title: Projects
icon: fas fa-folder-open
order: 4        # This puts it after Home/About, before Chat With Me
---

# My Projects

Here are some of the projects I've built during my learning journey:

### 1. Model Context Protocol (MCP) CLI Chatbot
**Description:** A command-line chatbot that uses Anthropic’s FastMCP SDK to connect Claude with custom tools, resources, and prompts.

**Tech Stack:** Python, FastMCP, UV, Anthropic Claude

**What I learned:** Clean tool integration, resources & prompts in MCP, reducing boilerplate for AI agents.

**[View Full Notes](https://thesahilsinh.github.io/posts/intro-to-mcp/)**  
**[GitHub Repo](https://github.com/thesahilsinh/AI-Learning/tree/main/Anth/Intro2MCP)**

---

### 2. QR.GEN — Real-time QR Code Studio

**Beautiful live QR code generator with logo overlay and scannability analysis.**

**Tech**: Flask • Python • Pillow • qrcode  
**Status**: Completed

<!-- **[View Project]()**   -->
**[GitHub](https://github.com/thesahilsinh/qr-gen)**